...
def read_table_csv(table_obj, csv_seperator=','):
    ...
    """
    df_rows = pd.read_csv(table_obj.csv_file_location, escapechar='\\', encoding='utf-8', quotechar='"',
                          sep=csv_seperator)
...
def prepare_single_table(schema_graph, table, path, max_distinct_vals=10000000, csv_seperator=',',
    ...
    if table == 'geotweet':
        for i in range(100):
            cat_attributes.append('tags{}'.format(i))
    elif table == 'gn':
        for i in range(100):
            cat_attributes.append('FEATUREN_NAME{}'.format(i))
    for i in range(100):
        cat_attributes.append('tags{}'.format(i))
    '''
    del_cat_attributes = []
    ...
    for rel_attribute in relevant_attributes:
        ...
        # categorical value
        if table_data.dtypes[attribute] == object or rel_attribute in cat_attributes:

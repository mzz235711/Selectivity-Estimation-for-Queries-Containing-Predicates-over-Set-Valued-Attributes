...
def _interval_probability_set(node, left, right, percent):
    p = 0
    if left[-1] >= node.unique_vals[-1]:
        p += (node.prob_sum[-1] - node.prob_sum[-2]) * percent[-1]
        left = left[:-1]
        right = right[:-1]
        percent = percent[:-1]
    lidx = -1 
    ...
    for i, v in enumerate(left):
        if v >= node.unique_vals[-1]:
            lidx = i - 1
            break
    for i, v in enumerate(right):
        if v <= node.unique_vals[0]:
            ridx = i + 1
    left = left[ridx:lidx]
    right = right[ridx:lidx]
    percent = percent[ridx:lidx]
    lower_idices = np.searchsorted(node.unique_vals, left, side='left')
    higher_idices = np.searchsorted(node.unique_vals, right, side='right')
    #if left >= node.unique_vals[-1] or right <= node.unique_vals[0]:
    #    return p
    #left = max(node.unique_vals[0], left)
    #right = min(node.unique_vals[-1], right)
    #if right >= node.unique_vals[-1]:
        #print(right, node.unique_vals[-1])
        #p += (node.prob_sum[-1] - node.prob_sum[-2])
        #return p
    higher_idices = np.minimum(higher_idices, [len(node.prob_sum) - 2] * len(higher_idices))
    lower_idices = np.minimum(lower_idices, [len(node.prob_sum) - 2] * len(lower_idices))
    searchadd = np.array([0] * (len(node.unique_vals) - 1))
    for i in range(len(higher_idices)):
        lower_idx = lower_idices[i]
        higher_idx = higher_idices[i]
        right_v = right[i]
        left_v = left[i]
        pc = percent[i]
        if lower_idx == higher_idx:
            searchadd[lower_idx - 1] += (right_v - left_v) * pc
        #p += (node.prob_sum[lower_idx] - node.prob_sum[lower_idx - 1]) * (right - left) / (node.unique_vals[lower_idx] - node.unique_vals[lower_idx - 1])
        #p += (node.prob_sum[lower_idx] - node.prob_sum[lower_idx - 1])
        else:
        #print(left, node.unique_vals[lower_idx - 1], node.unique_vals[lower_idx])
        #p += (node.prob_sum[lower_idx] - node.prob_sum[lower_idx - 1]) * (node.unique_vals[lower_idx] - left) / (node.unique_vals[lower_idx] - node.unique_vals[lower_idx - 1])
            searchadd[lower_idx - 1] += node.unique_vals[lower_idx] - left_v
            for j in range(lower_idx, higher_idx - 1):
                searchadd[j] += (node.unique_vals[j + 1] - node.unique_vals[j]) * pc
        #print(p)
        #p += node.prob_sum[higher_idx - 1] - node.prob_sum[lower_idx]
        #print(p)
        #print(right, node.unique_vals[higher_idx - 1], node.unique_vals[higher_idx])
        #if higher_idx < len(node.prob_sum) - 1 and higher_idx < len(node.unique_vals) - 1:
            if higher_idx < len(node.unique_vals):
                #p += (node.prob_sum[higher_idx] - node.prob_sum[higher_idx - 1]) * (right - node.unique_vals[higher_idx - 1]) / (node.unique_vals[higher_idx] - node.unique_vals[higher_idx - 1])
                searchadd[higher_idx - 1] = (right_v - node.unique_vals[higher_idx - 1]) * pc
    p += np.sum((node.prob_sum[1:-1] - node.prob_sum[:-2]) * searchadd / (node.unique_vals[1:] - node.unique_vals[:-1]))
        #else:
            #p += (node.prob_sum[higher_idx] - node.prob_sum[higher_idx] - 1)
        #print(left, right, node.unique_vals[0], node.unique_vals[-1], lower_idx, higher_idx)
        #print(p)
    return p

def _interval_probability(node, left, right, null_value, left_included, right_included, isset, islast):
    if left == -np.inf:
        lower_idx = 0
    else:
    ...
    #higher_idx = max(higher_idx, len(node.unique_vals))
    #print("left ", left)
    #print("right ", right)
    #print('lower_idx ', lower_idx)
    #print('higher_idx ', higher_idx)
    if isset:
        p = 0
        #higher_idx -= 1
        #lower_idx -= 1
        if islast is True and left >= node.unique_vals[-1]:
            p += (node.prob_sum[-1] - node.prob_sum[-2])
            return p
        if left >= node.unique_vals[-1] or right <= node.unique_vals[0]:
            return p
        ...
        right = min(node.unique_vals[-1], right)
            ...
            #p += (node.prob_sum[-1] - node.prob_sum[-2])
            #return p
        higher_idx = min(higher_idx, len(node.prob_sum) - 1)
        lower_idx = min(lower_idx, len(node.prob_sum) - 1)
        if lower_idx == higher_idx:
            p += (node.prob_sum[lower_idx] - node.prob_sum[lower_idx - 1]) * (right - left) / (node.unique_vals[lower_idx] - node.unique_vals[lower_idx - 1])
            #p += (node.prob_sum[lower_idx] - node.prob_sum[lower_idx - 1])
        else:
...
def identity_likelihood_range(node, data, dtype=np.float64, overwrite_ranges=None, **kwargs):
    ...
    percentages = glo.get_value('percentages')
    issetcol = glo.get_value('issetcol')
    probs = np.zeros((data.shape[0], 1), dtype=dtype)
    ranges = overwrite_ranges
    if overwrite_ranges is None:
        ranges = data[:, node.scope[0]]
    #print('SHAPE: ', ranges.shape)
    #print('DATA_SHAPE: ', data.shape)
    isset = issetcol[node.scope[0]]
    for i, rang in enumerate(ranges):
        ...
        #print("rang ", rang.get_ranges())
        '''
        if isset:
            percent = [1] * len(rang.get_ranges())
            if percentages[node.scope[0]] is not None:
                percent = percentages[node.scope[0]]
            left = np.array([i[0] for i in rang.get_ranges()])
            right = np.array([i[1] for i in rang.get_ranges()])
            probs[i] += _interval_probability_set(node, left, right, percent)
        '''
        if True:
            for k, interval in enumerate(rang.get_ranges()):
                inclusive = rang.inclusive_intervals[k]
                percent = 1
                islast = (k == (len(rang.get_ranges()) - 1))
                probs[i] += (_interval_probability(node, interval[0], interval[1], rang.null_value, inclusive[0],
                                              inclusive[1], isset, islast) * percent)
    #print(node.scope[0], probs[i])
    return probs

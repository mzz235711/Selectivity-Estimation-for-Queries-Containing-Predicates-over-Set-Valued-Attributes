...
class AQPSPN(CombineSPN, RSPN):
    ...
    def _parse_conditions(self, conditions, group_by_columns=None, group_by_tuples=None):
        ...
        percentages = np.array([None] * len(self.column_names))
        issetcol = np.array([False] * len(self.column_names))
        percentage_str = glo.get_value("percentage_str")
        if percentage_str is not None:
            percentages_str = percentage_str.split(";")
            for p_str in percentages_str:
                col = p_str.split(':')[0]
                values = p_str.split(':')[1]
                ps = [float(i) for i in values.split(',')]
                col_index = self.column_names.index(col)
                percentages[col_index] = ps
        glo.set_value("percentages", percentages)
        ...
        for (table, condition) in conditions:
            ...
            if self.meta_types[attribute_index] == MetaType.DISCRETE:
                ...
                if '<=' in condition:
                    ...
                    #for i in ranges[0]:
                    #    if i is NumericRange:
                    #        logger.info(f"RANGS: {i.ranges}")
                    #        logger.info(f"INTERVAL: {i.inclusive_intervals}")
                    #    elif i is NominalRange:
                    #        logger.info(f"VALUES: {i.possible_values}")
                    ...
                    #for i in ranges[0]:
                    #    if type(i) == NumericRange:
                    #        logger.info(f"RANGS: {i.ranges}")
                    #        logger.info(f"INTERVAL: {i.inclusive_intervals}")
                    #    elif type(i) == NominalRange:
                    #        logger.info(f"VALUES: {i.possible_values}")
                elif '=' in condition:
                    _, literal = condition.split('=', 1)
                    literal = float(literal.strip())
                    ...
                    literal_list, intervals = _literal_list_int(condition)
                    issetcol[attribute_index] = True
                    ranges = _adapt_ranges_scope(attribute_index, literal_list, intervals, ranges) 
                    #for i in ranges[0]:
                    #    if i is NumericRange:
                    #        logger.info(f"RANGS: {i.ranges}")
                    #        logger.info(f"INTERVAL: {i.inclusive_intervals}")
        ...
        if group_by_columns_merged is not None:
            for matching_group_by_idx, column in enumerate(group_by_columns_merged):
                ...
                if self.meta_types[attribute_index] == MetaType.DISCRETE:
                    ...
                    raise ValueError("Unknown Metatype")
        glo.set_value("issetcol", issetcol)
...
def _literal_list_int(condition):
    _, literals = condition.split('(', 1)
    invalues = [int(value.strip(' "\'')) for value in literals[:-1].split(',')]
    ranges = []
    intervals = []
    for i in range(0, len(invalues), 2):
        ranges.append([invalues[i], invalues[i + 1]])
        intervals.append([True, True])
    '''
    if (len(invalues) == 1):
        ranges.append([lb, lb + 1])
        intervals.append([True, False])
    else:
        last = lb
        for val in invalues[1:]:
            if val == last + 1:
              last = val
            else:
                ranges.append([lb, last + 1])
                intervals.append([True, False])
                lb = val
                last = lb
        ranges.append([lb, invalues[-1] + 1])
        intervals.append([True, False])
    '''
...
def _adapt_ranges_scope(attribute_index, literal_list, intervals, ranges):
    matching_none_intervals = [idx for idx, single_range in enumerate(ranges[:, attribute_index]) if
                               single_range is None]
    #side_ranges = []
    #inclusive_intervals = []
    #for i in range(0, len(literal_list), 2):
    #    side_ranges.append([literal_list[i], literal_list[i + 1]])
    #    inclusive_intervals.append([True, True])
    ranges[matching_none_intervals, attribute_index] = NumericRange(literal_list, inclusive_intervals=intervals)
    return ranges

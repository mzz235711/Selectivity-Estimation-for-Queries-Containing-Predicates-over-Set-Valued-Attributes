...
parser.add_argument('--freqtype', type=str)
...
class NeuroCard(tune.Trainable):
    ...
    def _setup(self, config):
        ...
        self.keepnum = args.keepnum
        self.hist = args.hist
        self.factorize_blacklist = []
        self.queries_csv = None
        self.percentage_csv = None
        self.querytype = args.querytype
        self.freqtype = args.freqtype
        self.settype = args.settype
        self.reload = args.reload
        self.checkpoint_to_load = None
        self.qid = 1
        if self.dataset == 'geotweet':
            self.qid = 7
        elif self.dataset == 'gn':
            self.qid = 6
        elif self.dataset == 'livejournal':
            self.qid = 16
        elif self.dataset == 'wiki':
            self.qid = 1
        if self.reload == 1:
            if self.hist == 1:
                self.checkpoint_to_load = "models/{}-settrie-{}-{}.pt".format(self.dataset, self.partnum, self.keepnum)
            elif self.hist == 2:
                self.checkpoint_to_load = "models/{}-color.pt".format(self.dataset)
            else:
                self.checkpoint_to_load = "models/{}-settrie-{}.pt".format(self.dataset, self.partnum, self.keepnum)
        if self.hist == 1:
            self.queries_csv = '../partition_{}_{}/{}_query_{}{}{}_trans_settrie_{}_approx.csv' \
                    .format(self.dataset, self.partnum, self.keepnum, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
            if self.settype != '_noset':
                self.percentage_csv = '../partition_{}_{}/{}_query_{}{}{}_trans_settrie_{}_approx_narupercent.csv' \
                        .format(self.dataset, self.partnum, self.keepnum, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
        elif self.hist == 2:
            self.queries_csv = '../graph_color/{}_query_{}{}{}_trans_settrie_{}.csv' \
                        .format(self.dataset, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
            if self.dataset == 'gn':
                self.partnum = 114
            elif self.dataset == 'geotweet':
                self.partnum = 832
                self.bs = 512
        else:
            self.queries_csv = '../color_partition_{}/{}_query_{}{}{}_trans_settrie_{}.csv' \
                        .format(self.dataset, self.partnum, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
        ...
        for i in range(self.partnum):
            if self.dataset == 'gn':
                if self.hist == 1:
                    #print("xxxxxxxxx")
                    self.factorize_blacklist.append('FEATURE_NAME{}'.format(i))
            elif self.dataset == 'geotweet':
                if self.hist == 1:
                    self.factorize_blacklist.append('tags{}'.format(i))
            elif self.dataset == 'bookc':
                if self.hist == 1:
                    self.factorize_blacklist.append('Users{}'.format(i))
            elif self.dataset == 'livejournal':
                if self.hist == 1:
                    self.factorize_blacklist.append('set{}'.format(i))
            elif self.dataset == 'wiki':
                if self.hist == 1:
                    self.factorize_blacklist.append('abstract{}'.format(i))
        print("CHECKPOINT_TO_LOAD: ", self.checkpoint_to_load)
        print("BLACK_LIST: ", self.factorize_blacklist)
        print("PARTITION_NUM: ", self.partnum)
        print("HIST: ", self.hist)
        print("QUERY_TYPE: ", self.querytype)
        print("SET_TYPE: ", self.settype)
        print("FREQ_TYPE: ", self.freqtype)
        print("DATASET: ", self.dataset)
        print("QUERY_PATH: ", self.queries_csv)
        print("PERCENT_PATH: ", self.percentage_csv)
        ...
        assert self.dataset in ['imdb', 'ukpoi', 'gn', 'geotweet', 'bookc', 'livejournal', 'wiki']
        ...
        elif self.dataset == 'ukpoi':
            table = datasets.LoadUkpoi(otype=self.otype)
        elif self.dataset == 'gn':
            table = datasets.LoadGN(otype=self.otype, partnum=self.partnum, keepnum = self.keepnum, hist=self.hist)
        elif self.dataset == 'geotweet':
            table = datasets.LoadGeoTweet(otype=self.otype, partnum=self.partnum, keepnum=self.keepnum, hist=self.hist)
        elif self.dataset == 'bookc':
            table = datasets.LoadBookc(otype=self.otype, partnum=self.partnum, keepnum=self.keepnum, hist=self.hist)
        elif self.dataset == 'livejournal':
            table = datasets.LoadLivejournal(otype=self.otype, partnum=self.partnum, keepnum=self.keepnum, hist=self.hist)
        elif self.dataset == 'wiki':
            table = datasets.LoadWiki(otype=self.otype, partnum=self.partnum, keepnum=self.keepnum, hist=self.hist)
        ...
        #self.querycorr = np.loadtxt(self.querycorr_csv)
        if (self.dataset == 'imdb' and len(self.join_tables) > 1) or self.dataset == 'ukpoi' or self.dataset == 'gn' \
                 or self.dataset == 'geotweet' or self.dataset == 'bookc' or self.dataset == 'livejournal' or self.dataset == 'wiki':
            queries_job_format = utils.JobToQuery(self.queries_csv)
            self.query_percents = utils.LoadPercentage(self.percentage_csv) if self.percentage_csv is not None else None 
    ...
    def MakeTableDataset(self, table):
        ...
        if self.factorize:
            train_data = common.FactorizedTable(
                train_data, word_size_bits=self.word_size_bits, factorize_blacklist=self.factorize_blacklist if self.factorize_blacklist else [])
        return train_data
    ...
    def MakeOrdering(self, table):
        fixed_ordering = None
        if self.dataset != 'imdb' and self.dataset != 'ukpoi' and self.dataset != 'gn' and self.dataset!= 'geotweet' \
                 and self.dataset != 'bookc' and self.dataset != 'livejournal' and self.dataset != 'wiki' and self.special_orders <= 1:
    ...
    def _train(self):
        ...
        for _ in range(min(self.epochs - self.epoch,
            ...
            mean_epoch_train_loss = run_epoch(
                ...
                self.model,
                self.opt,
                upto=self.max_steps if self.dataset == 'imdb' or self.dataset == 'ukpoi' or self.dataset == 'gn' \
                        or self.dataset == 'geotweet' or self.dataset == 'bookc' or self.dataset == 'livejournal' or self.dataset == 'wiki' else None,
        ...
        if self.compute_test_loss:
            returns['test_bits'] = np.mean(
                run_epoch(
                    ...
                    val_data=self.train_data,
                    batch_size=1024,
                    upto=None if self.dataset != 'imdb' and self.dataset != 'ukpoi' and self.dataset != 'gn' \
                             and self.dataset != 'geotweet' and self.dataset != 'bookc' and self.dataset != 'livejournal' and self.dataset != 'wiki' else 20,
                    log_every=200,
                    table_bits=self.table_bits,
                    return_losses=True,
                )) / np.log(2)
    ...
    def _save(self, tmp_checkpoint_dir):
        ...
        if self.hist == 1:
            PATH='models/{}-settrie-{}-{}.pt'.format(self.dataset, self.partnum, self.keepnum)
        elif self.hist == 2:
            PATH = 'models/{}-color.pt'.format(self.dataset)
        else:
            PATH='models/{}-settrie-{}.pt'.format(self.dataset, self.partnum)
        torch.save(self.model.state_dict(), PATH)
        wandb.save(PATH)
        print('Saved to:', PATH)
        return {'path': PATH}
    ...
    def Query(self,
        ...
        for est in estimators:
            est_card = est.Query(cols, ops, vals, percent)
    ...
    def evaluate(self, num_queries, done, estimators=None):
        ...
        if num_queries:
            ...
            for i in range(num_queries):
                ...
                percent = self.query_percents[i] if self.query_percents is not None else None
            ...
            for est in estimators:
                ...
                est.report_infile(self.dataset, self.partnum, self.keepnum, self.querytype, self.freqtype, self.settype, self.hist)

                series = pd.Series(est.query_dur_ms)
                print(series.describe())
                series.to_csv(str(est) + '.csv', index=False, header=False)
                if self.hist == 0:
                    result_dir = './result'.format(self.dataset, self.partnum)
                    if not os.path.exists(result_dir):
                        os.makedirs(result_dir)
                    resultname = './result/{}_query_{}{}{}_{}_result_neurocard.csv' \
                            .format(self.dataset, self.partnum, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
                else:
                    result_dir = './result/'.format(self.dataset, self.partnum, self.keepnum)
                    if not os.path.exists(result_dir):
                        os.makedirs(result_dir)
                    resultname = './result/{}_query_{}{}{}_{}_result_neurocard.csv' \
                            .format(self.dataset, self.partnum, self.keepnum, self.dataset, self.qid, self.freqtype, self.settype, self.querytype)
                np.savetxt(resultname, est.est_cards, fmt='%d')

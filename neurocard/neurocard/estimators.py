...
class CardEst(object):
    ...
    def report_infile(self, dataset, partnum, keepnum, querytype, freqtype, settype, hist):
        ...
        resultfile.write('dataset: ' + dataset + '\n')
        resultfile.write('querytype: ' + querytype + '\n')
        resultfile.write('freqtype: ' + freqtype + '\n')
        resultfile.write('settype: ' + settype + '\n')
        resultfile.write('partnum: ' + str(partnum) + '\n')
        resultfile.write('keepnum: ' + str(keepnum) + '\n')
        resultfile.write('hist: ' + str(hist) + '\n')
        resultfile.write('median_error: ' + str(np.quantile(est.errs, 0.5)) + '\n')
        resultfile.write('95th_error: ' + str(np.quantile(est.errs, 0.95)) + '\n')
        resultfile.write('99th_error: ' + str(np.quantile(est.errs, 0.99)) + '\n')
        resultfile.write('max_error: ' + str(np.max(est.errs)) + '\n')
        resultfile.write('mean_error: ' + str(np.mean(est.errs)) + '\n')
        resultfile.write('time_ms: ' + str(np.mean(est.query_dur_ms)) + '\n\n')
        resultfile.close()
        #np.savetxt('error.csv', est.errs)
...
def FillInUnqueriedColumns(table, columns, operators, vals, percents=None):
    ...
    cs = table.columns
    os, vs, ps = [None] * ncols, [None] * ncols, [None] * ncols
...
def HandlePercents(fact_table, columns, operators, vals, percents):
    fact_percents = []
    n_fact_cols = len(fact_table.columns)
    for i in range(nfact_cols):
        op = operators[i]
        percent = percents[i]
        val = vals[i]
        if percent is None:
            fact_percents.append(1)
        else:
            fact_percents.append([1] * columns[i].distribution_size)
            for idx in range(len(op)):
                v = val[idx]
                v_array = np.array(list(v))
                indexes = common.Discretize(column[i], v_array, fail_out_of_domain=False)
                for ix, p in zip(indexes, percent):
                    fact_percents[-1][ix] = p
    return fact_percents

def ProjectQuery(fact_table, columns, operators, vals, percents):
    ...
    for i in range(nfact_cols):
        ...
        if op is None:
            ...
            fact_percents.append([1] * col.distribution_size)
            #for o, v, p in zip(op, val, percent):
            for idx in range(len(op)):
                o = op[idx]
                v = val[idx]
                if percent is not None:
                    if o == 'IN':
                        ...
                        v_array = np.array([v])
                    indexes= common.Discretize(columns[j], v_array, fail_out_of_domain=False)
                    for ix, p in zip(indexes, percent):
                        fact_percents[-1][ix] = p
    ...
    assert len(fact_cols) == len(fact_ops) == len(fact_vals) == len(
        fact_dominant_ops)
    return fact_cols, fact_ops, fact_vals, fact_dominant_ops, fact_percents
...
class ProgressiveSampling(CardEst):
    ...
    def _sample_n(self,
        ...
        for i in range(ncols):
            ...
            if self.shortcircuit and operators[natural_idx] is None:
                ...
                if percents[i] is not None:
                    percent_i = torch.as_tensor(percents[i], device=self.device)
                else:
                    percent_i = torch.as_tensor(1, device=self.device)
                #print("PERCNET_I")
                #print(percent_i)
                num_classes = len(dvs)
    ...
    def _StandardizeQuery(self, columns, operators, vals, percents):
        cs, os, vs, ps = FillInUnqueriedColumns(self.table, columns, operators, vals, percents)
        return HandlePercents(self.table, cs, os, vs, ps) 

    def Query(self, columns, operators, vals, percents):
        torch.cuda.empty_cache()

        # Massages queries into natural order.
        columns, operators, vals, percents = self._StandardizeQuery(
            columns, operators, vals, percents)
...
class FactorizedProgressiveSampling(ProgressiveSampling):
    ...
    def _StandardizeQuery(self, columns, operators, vals, percents):
        ...
        cols, ops, vals, percents = FillInUnqueriedColumns(self.base_table, columns,
                                                 operators, vals, percents)
        #print('PERCENTS: ', percents)
        cols, ops, vals, dominant_ops, percents = ProjectQuery(self.fact_table, cols, ops,
                                                     vals, percents)
        self.dominant_ops = dominant_ops
        return cols, ops, vals, percents
...
class JoinSampling(CardEst):
    ...
    def _StandardizeQuery(self, columns, operators, vals, percents):
        return FillInUnqueriedColumns(self.table, columns, operators, vals, percents)
